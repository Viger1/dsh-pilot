...
**引擎共享方式（v0 决策，已被证伪）**：当初决定直接复制 `browser.ts`（约 200 行）独立演化，理由是「抽公共包会把两个未稳定的 API 绑死，代价只是短期内修 bug 要同步两处，可接受」。

**实际代价比预估的大。** 2026-08-17：`dsh-review` 审查 `dsh-design`（第三份复制）时发现「取消在浏览器启动窗口内被忽略」——abort 只派发一次，而监听器在 `newPage()` 之后才注册，于是那段窗口内的取消永远不会触发，页面照常加载到超时。核实后确认**同一缺陷在 preview 与 pilot 中同样存在**，只是它们各自的审查没抽到这条。也就是说：复制引擎时把缺陷也复制了三份，而修复不会自动同步，是否修掉取决于哪个仓库的审查运气好。三处已一并修复。

**修订决策**：引擎应抽成共享实现。两种取舍——
- **库包**（普通 npm 依赖）：消除维护重复，零安装摩擦，但运行时仍是各自的浏览器进程。
- **能力接缝**（`ctx.browser` 的 Service Definition + Provider，dsh 官方推崇的写法）：同时消除重复与多进程，但消费者需要 provider 被组合进 profile，对已发布的插件是破坏性变更。

倾向：先抽库包拿到「一处修复处处生效」，再评估接缝，避免破坏已在 npm 上的 preview/pilot。
